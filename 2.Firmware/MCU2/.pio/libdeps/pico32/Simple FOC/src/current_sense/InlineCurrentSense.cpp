...
InlineCurrentSense::InlineCurrentSense(float _shunt_resistor, float _gain, int _pinA, int _pinB, int _pinC){
    ...
    volts_to_amps_ratio = 1 / 0.667f / 0.2f * 2;
...
int InlineCurrentSense::driverAlign(float voltage){
    ...
    if(_isset(pinA)){
        ...
        // printf("log in 2222 c.a:%0.3f c.b:%0.3f c.c:%0.3f\n",
        // c.a, c.b, c.c);

        // read the current 100 times ( arbitrary number )
        for (int i = 0; i < 100; i++) {
            PhaseCurrent_s c1 = getPhaseCurrents();
            // printf("log in [%d] c.a:%0.3f c.b:%0.3f c.c:%0.3f\n",
            //     i, c.a, c.b, c.c);
        ...
        // printf("log in c.a:%0.3f c.b:%0.3f c.c:%0.3f ab_ratio:%0.3f ac_ratio:%0.3f\n",
        // c.a, c.b, c.c, ab_ratio, ac_ratio);
    ...
    if(_isset(pinB)){
        ...
        if(_isset(pinA) && ba_ratio > 1.5f ){ // should be ~2
            ...
            gain_b *= _sign(c.c);
            exit_flag = 2; // signal that pins have been switched
            // printf("log in 99999999\n");
        }else{
            // error in current sense - phase either not measured or bad connection
            // printf("log in 1000000\n");
            return 0;
        }   
    }
