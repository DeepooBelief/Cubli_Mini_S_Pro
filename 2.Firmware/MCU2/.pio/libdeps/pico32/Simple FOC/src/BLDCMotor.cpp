...
int  BLDCMotor::initFOC() {
  int exit_flag = 1;
  sensor_init_res = FOCMotorInitStatus::motor_sense_ok;
  voltage_sensor_align = 0.5f;
  ...
  if(exit_flag == 0){
    sensor_init_res = FOCMotorInitStatus::motor_encoder_fail;
  }
  ...
  if(exit_flag){
    if(current_sense){ 
      ...
      sensor_init_res = FOCMotorInitStatus::motor_current_sense_fail; 
    }
